...
/* Parser Rules: starts with small letters */
// value_attr is for nodes in AST
...
expr returns [value_attr = str(), type_attr = str()]:
    expr '+' term        #expr_term_plus // label for rule
    | expr '-' term      #expr_term_minus
    | expr RELOP term    #expr_term_relop // true, false e.g. 1 < 2
    | term               #term4
    ;
...
/* Lexical Rules: starts with capitals */
INT     : DIGIT+ ;
