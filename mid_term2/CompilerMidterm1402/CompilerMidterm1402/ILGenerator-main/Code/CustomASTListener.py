...
class ASTListener(AssignmentStatementListener):
    def __init__(self):
        ...
        self.max_x = 0
        self.scoped_operators = ['if','for','while','block','case']
    ...
    def make_AST_subtree(self, tree_node, operator):
        ...
        if operator is "switch":
            first_child = self.ast.make_node("{", child=None, brother=first_child)
            brother.value_attr.brother = self.ast.make_node(value="}", child=None, brother=None)
        sub_tree_pntr = self.ast.make_node(
            value=operator, child=first_child, brother=None
        )
