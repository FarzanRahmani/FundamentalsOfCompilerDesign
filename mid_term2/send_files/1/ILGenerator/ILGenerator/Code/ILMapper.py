class ILMapper:
    ...
    binary_operators = ['>=', '>', '==', '!=', '<', '<=', '&&', '||', '+', '-', '*', '/', '=', '&&', '||', '>>', '<<', ]
    flow_control_operators = ['if', 'for', 'while', 'block', 'switch'] # switch
    ...
    def switch_statement(self):
        ...
        for _ in range(len(stack_array)):
        ...
        # switch_label = self.create_new_label()
        # end_label = self.create_new_label()
        
        # condition = self.stack.pop()  # Assuming the switch statement condition is on top of the stack
        # switch_code = f"{condition}\n"  # Generate code for evaluating the switch condition
        
        # cases = []  # Placeholder for storing cases
        
        # current_code = self.il_codes.pop()
        # while current_code != 'begin':
        #     if current_code.startswith('case'):
        #         cases.append(current_code)
        #     current_code = self.il_codes.pop()
        
        # # Generate code for each case in the switch statement
        # for case_code in reversed(cases):
        #     case_value = case_code.split()[1]  # Extract the case value from the code
        
        #     # Generate code for comparing the switch condition with the case value
        #     switch_code += f"{condition}\nldc.i8 {case_value}\nceq\nbrtrue {switch_label}\n"
        
        # # If no case matches, jump to the end label
        # switch_code += f"br {end_label}\n"
        
        # # Generate code for the switch body (statements within the switch)
        # switch_code += f"{switch_label}:\n"
        # while len(self.il_codes) > 0:
        #     switch_code += self.il_codes.pop()
        
        # switch_code += f"{end_label}:\n"  # Label at the end of the switch statement
        
        # return switch_code

        # -----------------------------------------------------------

        # # Retrieve necessary values from the stack
        # closing_brace = self.il_codes.pop()  # Pop the closing brace '}'
        # case_blocks = []  # To store case blocks
        # default_block = None  # To store the default block if present
        # switch_id = self.stack.pop()  # Assuming ID is on top of the stack

        # # Parse the cases and default block
        # current_code = self.il_codes.pop()
        # while current_code != 'switch':
        #     if current_code == 'case':
        #         # Parse case block
        #         case_condition = self.stack.pop()
        #         case_label = self.create_new_label()
        #         case_blocks.append((case_condition, case_label))
        #         current_code = self.il_codes.pop()
        #     elif current_code == 'default':
        #         # Parse default block
        #         default_label = self.create_new_label()
        #         default_block = default_label
        #         current_code = self.il_codes.pop()
        #     else:
        #         current_code = self.il_codes.pop()

        # # Generate IL code for switch statement
        # switch_start_label = self.create_new_label()
        # switch_end_label = self.create_new_label()
        # result = f"ldloc {switch_id}\n"
        # for condition, label in case_blocks:
        #     result += f"ldc.i8 {condition}\nceq\nbrtrue {label}\n"
        # if default_block:
        #     result += f"br {default_block}\n"
        # else:
        #     result += f"br {switch_end_label}\n"

        # result += f"{switch_start_label}:\n"

        # # Generate IL code for case blocks
        # for _, label in case_blocks:
        #     result += f"{label}:\n"
        #     result += self.il_codes.pop()  # Retrieve IL code for each case

        # # Generate IL code for default block if present
        # if default_block:
        #     result += f"{default_block}:\n"
        #     result += self.il_codes.pop()  # Retrieve IL code for default block

        # result += f"{switch_end_label}:\n"

        # return result + closing_brace  # Combine the generated IL code with the closing brace '}'
        ...
        # expr = temp_switch_stack.pop() # switch expression
        # cases = []
        # default = None
        # while len(temp_switch_stack) > 0:
        #     code = temp_switch_stack.pop()
        #     if code.startswith('case'):
        #         cases.append(code)
        #     elif code == 'default':
        #         default = temp_switch_stack.pop()
        ...
        #     result += f'brfalse {label}\n'
        #     result += case + '\n' 
        #     result += f'br {end_label}\n'
        #     result += f'{label}:\n'
